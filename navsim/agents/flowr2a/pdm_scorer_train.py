"""
Training-time PDMScorer that additionally saves collision track IDs to self.

This is a copy of the original PDMScorer from
navsim/planning/simulation/planner/pdm_planner/scoring/pdm_scorer.py
with minimal patches to expose:
  - self.proposal_collided_track_ids  (all collided track IDs per proposal)
  - self.proposal_fault_collided_track_ids  (at-fault collision track IDs per proposal)
  - self.temp_collided_track_ids  (all TTC-collided track IDs per proposal)
  - self.ttc_collided_track_ids  (TTC infraction track IDs per proposal)

These are needed for iPad-style GT extraction without importing iPad's separate scorer.
"""
...
from navsim.planning.simulation.planner.pdm_planner.scoring.pdm_comfort_metrics import ego_is_comfortable as ego_is_comfortable_v1
from navsim.planning.simulation.planner.pdm_planner.scoring.pdm_comfort_metrics_v2 import ego_is_comfortable as ego_is_comfortable_v2
...
class PDMScorer:
    ...
    Training variant: additionally saves collision track IDs to self for iPad GT extraction.
    """
    ...
    def __init__(
        ...
        ttc_bound: float = 1.5,
        use_history_comfort: bool = False,
        driving_direction_exclude_intersection: bool = False,
    ):
        """
        Constructor of PDMScorer
        :param proposal_sampling: Sampling parameters for proposals
        :param use_history_comfort: If True, use v2-style history-padded comfort metric
        :param driving_direction_exclude_intersection: If True, exclude intersection from DDC (v2 behavior)
        """
        self.proposal_sampling = proposal_sampling
        self._config = config
        self._vehicle_parameters = vehicle_parameters
        self._ttc_bound = ttc_bound
        self._use_history_comfort = use_history_comfort
        self._driving_direction_exclude_intersection = driving_direction_exclude_intersection
    ...
    def score_proposals(
        ...
        :param human_past_trajectory: past ego trajectory for history comfort (v2)
    ...
    def score_proposals_ttc(
        ...
        """
        Run only TTC calculation and return TTC-related results.
        :return: (ttc_scores, per_step_ttc) where
            ttc_scores: (N,) binary TTC metric per proposal
            per_step_ttc: (N, T+1) continuous TTC in seconds per timestep
        """
        self._reset(states, observation, centerline, route_lane_ids, drivable_area_map)
        self._calculate_ego_area()
        self._calculate_ttc()
        return (
            self._weighted_metrics[WeightedMetricIndex.TTC].copy(),
            self._per_step_ttc.copy(),
        )

    def score_proposals_ttc_ep(
        self,
        states: npt.NDArray[np.float64],
        observation: PDMObservation,
        centerline: PDMPath,
        ...
        """
        Run only TTC + ego progress calculation.
        :return: None (results stored in self for extraction by _pairwise_subscores_ttc_ep)
        """
        self._reset(states, observation, centerline, route_lane_ids, drivable_area_map)
        self._calculate_ego_area()
        self._calculate_ttc()
    ...
    def _reset(
        ...
        # [PATCH] Initialize collision track ID dicts for iPad GT extraction
        ...
        # [PATCH] Per-timestep continuous TTC: stores smallest projection delta (in seconds)
        # where an at-fault TTC collision is detected. self._ttc_bound = no collision (max bound).
        self._per_step_ttc = np.full(
    ...
    def _calculate_no_at_fault_collision(self) -> None:
        ...
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            ...
            for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                ...
                if (self._observation.red_light_token in token) or (token in self.proposal_collided_track_ids[proposal_idx]):
                    continue
                ...
                if collisions_at_stopped_track_or_active_front or (
                    ...
                    # [PATCH] Record at-fault collision token
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        if self._driving_direction_exclude_intersection:
            for proposal_idx in range(self._num_proposals):
                for time_idx in range(self.proposal_sampling.num_poses + 1):
    ...
    def _calculate_ttc(self):
        ...
        # [PATCH] Track TTC infraction tokens per proposal
        self.ttc_collided_track_ids = {
            proposal_idx: [] for proposal_idx in range(self._num_proposals)
        }

        # calculate TTC with configurable lookahead bounded by self._ttc_bound.
        # E.g. ttc_bound=1.5 → max_idx=15 → [0,3,6,9,12] (max 1.2s)
        #      ttc_bound=2.0 → max_idx=20 → [0,3,6,9,12,15,18] (max 1.8s)
        max_idx = int(round(self._ttc_bound / self.proposal_sampling.interval_length))
        future_time_idcs = np.arange(0, max_idx, 3)
        # Max valid observation index for clamping (extended projections may exceed bounds)
        max_obs_idx = len(self._observation._global_to_local_idcs) - 1
        ...
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                current_time_idx = min(time_idx + future_time_idx, max_obs_idx)
                ...
                for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                    ...
                    if is_agent_ahead(ego_rear_axle, track_state) or (
                        ...
                        # [PATCH] Record per-step continuous TTC (smallest projection delta)
                        delta_t = float(future_time_idx) * self.proposal_sampling.interval_length
                        self._per_step_ttc[proposal_idx, time_idx] = min(
                            self._per_step_ttc[proposal_idx, time_idx], delta_t
                        )
                        # [PATCH] Record TTC infraction token
    ...
    def _calculate_is_comfortable(self) -> None:
        ...
        When use_history_comfort is enabled and past trajectory is available,
        pads proposal states with human history states (v2 behavior).
        """
        if self._use_history_comfort:
        ...
        is_comfortable = ego_is_comfortable_v1(self._states, time_point_s)
    ...
    def _calculate_history_comfort(self) -> None:
        """
        V2-style comfort metric, padded with past human trajectory states.
        Detects kinematic discontinuities at the junction between human history
        and predicted trajectory via Savitzky-Golay filtered acceleration/jerk.
        """
        ...
        if self._human_past_trajectory is not None:
            ...
            is_history_comfortable = ego_is_comfortable_v2(padded_states_array, time_point_s).all(axis=-1)

        self._weighted_metrics[WeightedMetricIndex.COMFORTABLE] = is_history_comfortable
