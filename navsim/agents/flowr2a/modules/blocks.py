...
class SinusoidalPosEmb(nn.Module):
    """Sinusoidal positional embedding for a continuous scalar (e.g. diffusion timestep
    or a continuous reward value). Input shape (...), output shape (..., dim)."""
    ...
    def forward(self, x):
        ...
        # Frequencies: exp(arange(0, half_dim) * -(log(10000) / (half_dim - 1)))
        emb = math.log(10000) / (half_dim - 1)
        emb = torch.exp(torch.arange(half_dim, device=device) * -emb)

        # x[..., None] -> (..., 1); emb broadcasts over the leading dims, giving (..., half_dim)
        emb = x[..., None] * emb

        # Concatenate sin and cos along the last dim
        emb = torch.cat((emb.sin(), emb.cos()), dim=-1)

        # Pad one zero if dim is odd (dim is recommended to be even)
        if self.dim % 2 == 1:
