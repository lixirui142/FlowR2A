...
def transfuser_loss(
    ...
    scorer_only = config.scorer_only if hasattr(config, "scorer_only") else False
    
    if scorer_only:
        # Stage 2 loss
        loss_dict = dict()
        if 'score_loss' in predictions:
            score_loss = predictions['score_loss']
            loss = config.score_weight * score_loss
            loss_dict["score_loss"] = score_loss
            if "score_sub_loss_dict" in predictions:
                loss_dict.update(predictions["score_sub_loss_dict"])
            loss_dict["loss"] = loss
        else:
            loss_dict["loss"] = 0.
        
        for k, v in predictions.items():
            if k.startswith("val_pdm_"):
                loss_dict[k] = v
        return loss_dict
    ...
    elif "trajectory" in predictions:
    ...
    if 'score_loss' in predictions:
        score_loss = predictions['score_loss']
        loss = loss + config.score_weight * score_loss
        loss_dict["score_loss"] = score_loss
        if "score_sub_loss_dict" in predictions:
            loss_dict.update(predictions["score_sub_loss_dict"])
    if "transfuser_trajectory" in predictions:
        transfuser_trajectory_loss = F.l1_loss(predictions["transfuser_trajectory"], targets["trajectory"])
        loss = loss + config.transfuser_trajectory_weight * transfuser_trajectory_loss
        loss_dict["transfuser_trajectory_loss"] = transfuser_trajectory_loss
    loss_dict["loss"] = loss

    # Pass through validation PDM metrics for logging
    for k, v in predictions.items():
        if k.startswith("val_pdm_"):
            loss_dict[k] = v
...
def _get_ce_cost(gt_valid: torch.Tensor, pred_logits: torch.Tensor) -> torch.Tensor:
    ...
    #nan solution. https://github.com/hustvl/DiffusionDrive/issues/76s
