...
class MetricCacheProcessor:
    ...
    def compute_metric_cache(self, scenario: AbstractScenario) -> Optional[CacheMetadataEntry]:
        ...
        # build past human trajectory from history frames (at 2Hz: -1.5s, -1.0s, -0.5s, 0s)
        num_history = scenario._scene_data.num_history_frames  # typically 4
        past_ego_states = [
            scenario.get_ego_state_at_iteration(it)
            for it in range(-(num_history - 1), 1)
        ]
        past_human_trajectory = InterpolatedTrajectory(past_ego_states)
