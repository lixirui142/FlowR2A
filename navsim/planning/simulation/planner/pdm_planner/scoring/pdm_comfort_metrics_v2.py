...
def ego_is_comfortable(
    ...
    """
    Accumulates all within-bound comfortability metrics (v2 implementation).
    Uses fixed savgol window sizes (8 for accel, 15 for jerk/yaw) and
    center-of-mass acceleration instead of rear-axle.
    :param states: array representation of ego state values
    :param time_point_s: time steps [s] of time dim
    :param vehicle_parameters: parameters of vehicle
    :return: boolean array (n_batch, 6) for each comfort metric
    """
    n_batch, n_time, n_states = states.shape
