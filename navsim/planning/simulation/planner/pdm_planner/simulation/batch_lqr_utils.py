...
EVAL = bool(int(os.environ["EVAL"])) if "EVAL" in os.environ else False

# When True, use torch.linalg.pinv + corrected banded difference matrix.
# When False, fall back to original navsim official behavior (np.linalg.pinv + buggy banded matrix).
# Set LQR_FIX to False when EVAL=True to reproduce official evaluation behavior
# See issue https://github.com/autonomousvision/navsim/issues/136 for details.
LQR_FIX = False if EVAL else True

# Fine-grained control: override individual fixes via env vars.
#   BANDED_FIX=0/1  — banded difference matrix fix (+=  vs =)
#   SOLVER_FIX=0/1  — torch pinv + lstsq fallback vs original torch pinv only
# When not set, both follow LQR_FIX.
BANDED_FIX = bool(int(os.environ["BANDED_FIX"])) if "BANDED_FIX" in os.environ else LQR_FIX
SOLVER_FIX = bool(int(os.environ["SOLVER_FIX"])) if "SOLVER_FIX" in os.environ else LQR_FIX
...
def _make_banded_difference_matrix(number_rows: int) -> npt.NDArray[np.float64]:
    ...
    if BANDED_FIX:
        # Fixed: use += to avoid the second assignment overwriting the first.
        # See https://github.com/autonomousvision/navsim/issues/52
        banded_matrix[:, 1:] += eye
        banded_matrix[:, :-1] += -eye
    else:
        # Original navsim official (buggy): second assignment overwrites overlapping entries.
        banded_matrix[:, 1:] = eye
        banded_matrix[:, :-1] = -eye
...
def _fit_initial_velocity_and_acceleration_profile(
    ...
    if SOLVER_FIX:
        try:
            # Use torch.linalg.pinv which is numerically stable across numpy versions.
            intermediate_solution = batch_matmul(
                torch.linalg.pinv(
                    torch.tensor(batch_matmul(A_T, A) + jerk_penalty * batch_matmul(R_T, R), dtype=torch.float64)
                ).numpy(),
                A_T,
            )
        except (RuntimeError, torch._C._LinAlgError) as e:
            # Fallback to PyTorch lstsq (stacked least squares) to avoid squaring the condition number
            # We solve A_stacked * S = I_stacked to get the intermediate matrix S where x = S * y
            print(f"Warning: PyTorch pinv failed ({e}), falling back to stacked lstsq.")
            
            A_tensor = torch.tensor(A, dtype=torch.float64)
            R_tensor = torch.tensor(R, dtype=torch.float64)
            
            # 1. Stack A and sqrt(penalty) * R
            R_scaled = (jerk_penalty ** 0.5) * R_tensor
            A_stacked = torch.cat([A_tensor, R_scaled], dim=1)
            
            # 2. Build the right-hand side to solve for the intermediate projection matrix
            dim_y = A.shape[1]
            dim_R = R.shape[1]
            
            I_tensor = torch.eye(dim_y, dtype=torch.float64).expand(batch_size, dim_y, dim_y)
            zeros_tensor = torch.zeros((batch_size, dim_R, dim_y), dtype=torch.float64)
            I_stacked = torch.cat([I_tensor, zeros_tensor], dim=1)
            
            try:
                # lstsq handles ill-conditioned matrices gracefully
                lstsq_result = torch.linalg.lstsq(A_stacked, I_stacked)
                intermediate_solution = lstsq_result.solution.numpy()
            except Exception as fallback_e:
                # Last resort: return zeros (results in constant velocity)
                print(f"Warning: Fallback lstsq also failed ({fallback_e}), returning constant velocity.")
                intermediate_solution = np.zeros((batch_size, A.shape[2], A.shape[1]), dtype=np.float64)
        x = np.einsum("bij, bj -> bi", intermediate_solution, y)
    else:
        # Original navsim official: np.linalg.pinv (unstable on numpy <= 1.23 for large matrices).
        # intermediate_solution = batch_matmul(
...
def _fit_initial_curvature_and_curvature_rate_profile(
    ...
    if SOLVER_FIX:
        try:
            # Use torch.linalg.pinv which is numerically stable across numpy versions.
            intermediate = batch_matmul(
                torch.linalg.pinv(
                    torch.tensor(batch_matmul(A_T, A) + Q, dtype=torch.float64)
                ).numpy(),
                A_T,
            )
        except (RuntimeError, torch._C._LinAlgError) as e:
            ...
            # Q is already the equivalent of R^T * R. Since Q is diagonal, its "square root" 
            # is just the element-wise square root of its diagonal entries.
            Q_sqrt = np.sqrt(Q)
            R_scaled = torch.tensor(Q_sqrt, dtype=torch.float64).unsqueeze(0).expand(batch_dim, -1, -1)
            
            # 1. Stack A and R_scaled along the row dimension
            A_stacked = torch.cat([A_tensor, R_scaled], dim=1)
            
            # 2. Build the right-hand side to solve for the intermediate projection matrix:
            # Solving A_stacked * S = I_stacked yields exactly S = (A^T A + Q)^-1 A^T
            I_tensor = torch.eye(dim, dtype=torch.float64).expand(batch_dim, dim, dim)
            zeros_tensor = torch.zeros((batch_dim, dim, dim), dtype=torch.float64)
            I_stacked = torch.cat([I_tensor, zeros_tensor], dim=1)
            
            try:
                # lstsq robustly handles the decomposition without forming A^T A
                lstsq_result = torch.linalg.lstsq(A_stacked, I_stacked)
                intermediate = lstsq_result.solution.numpy()
            except Exception as fallback_e:
                # Last resort: use zero curvature (straight line approximation)
                print(f"Warning: Fallback lstsq also failed ({fallback_e}), returning zero curvature.")
                intermediate = np.zeros_like(A_T)
