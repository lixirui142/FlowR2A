...
from navsim.visualization.config import BEV_PLOT_CONFIG, NEW_TAB_10, TRAJECTORY_CONFIG, CAMERAS_PLOT_CONFIG
...
def get_traj_plot_config(index, color_map = NEW_TAB_10):
    return {
        "fill_color": color_map[index],
        "fill_color_alpha": 1.0,
        "line_color": color_map[index],
...
def plot_bev_with_trajectories(scene: Scene, trajectories, add_gt = False) -> Tuple[plt.Figure, plt.Axes]:
    """
    Plots agent and human trajectory in birds-eye-view visualization
    :param scene: navsim scene dataclass
    :param agent: navsim agent
    ...
    for i, trajectory in enumerate(trajectories):
        add_trajectory_to_bev_ax(ax, trajectory, get_traj_plot_config(i))
        # add_trajectory_to_bev_ax(ax, agent_trajectory, TRAJECTORY_CONFIG["agent"])
    if add_gt:
        add_trajectory_to_bev_ax(ax, human_trajectory, TRAJECTORY_CONFIG["human"])
    configure_bev_ax(ax)
...
def frame_plot_to_pil_trajectories(
    scene: Scene,
    trajectories_list,
    add_gt = False
) -> List[Image.Image]:
    ...
    for trajectories in tqdm(trajectories_list, desc="Rendering frames"):
        fig, ax = plot_bev_with_trajectories(scene, trajectories, add_gt = add_gt)
...
def frame_plot_to_gif_with_trajectories(
    file_name: str,
    scene: Scene,
    trajectories_list,
    duration: float = 500,
    ...
    """
    images = frame_plot_to_pil_trajectories(scene, trajectories_list, add_gt = add_gt)
